...
import { fetchCoreCustomers } from '../../api/index'
...
function ProfileRow({ label, value, color }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 12px', background: 'var(--surface, #F8FAFB)', borderRadius: 8 }}>
      ...
      <span style={{ fontSize: 13, fontWeight: 700, color: color ?? 'var(--ink-2)' }}>{value ?? '—'}</span>
    </div>
  )
}

export default function CoreCustomerProfile({ startAt, endAt }) {
  ...
  return (
    <div className="card">
      ...
      <div className="card-b">
        {!hasData ? (
          <p style={{ margin: 0, fontSize: 13, color: 'var(--muted-2)' }}>분석된 고객 데이터가 없습니다.</p>
        ) : (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
              <div style={{ width: 48, height: 48, borderRadius: 14, background: gender ? GENDER_STYLE[gender]?.bg : 'var(--accent-soft)', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 3 }}>오늘 가장 많이 방문한 고객층</div>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <ProfileRow label="성별" value={gender} color={gender ? GENDER_STYLE[gender]?.fg : undefined} />
              <ProfileRow label="연령대" value={age} />
            </div>
