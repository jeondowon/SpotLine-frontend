...
// 운영 현황 통합 카드
import DwellAndLostCard from "../components/dashboard/DwellAndLostCard";
...
import MarketingRecommendationsCard from "../components/dashboard/MarketingRecommendationsCard";
    ...
    <div className="app">
      ...
      <div className="main">
        ...
        <div className="content">
          ...
          <DwellAndLostCard startAt={startAt} endAt={endAt} />

          {/* 60일 방문 추세 */}
          <VisitTrendCard startAt={trendStartAt} endAt={endAt} selectedDay={day} />
          ...
          >
            <div style={{ padding: "32px 32px", display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginBottom: 10 }}>
                <span style={{ fontSize: 26, fontWeight: 800, color: "var(--accent-ink)", lineHeight: 1 }}>15</span>
                <span style={{ fontSize: 12.5, color: "var(--muted)" }}>개의 잠긴 인사이트가 발견됐어요</span>
              </div>
              <h2 style={{ margin: "0 0 10px", fontSize: 21, fontWeight: 800, color: "var(--ink)", letterSpacing: "-0.025em", lineHeight: 1.4 }}>
                왜 어떤 날은 잘 되고,<br />어떤 날은 안 될까요?
              </h2>
              <p style={{ margin: "0 0 20px", fontSize: 13, color: "var(--muted)", lineHeight: 1.75 }}>
                알림 기능, 구역별 체류 분석, 방문자 특성까지<br />
                더욱 전문적인 분석을 기반으로 프리미엄이 해답을 찾아드릴게요.
              </p>
              <button
                onClick={() => setIsPremiumModalOpen(true)}
                style={{ alignSelf: "flex-start", padding: "11px 20px", fontSize: 13.5, fontWeight: 700, color: "#fff", background: "var(--ink)", border: "none", borderRadius: 999, cursor: "pointer", letterSpacing: "-0.01em" }}
              >
                프리미엄 시작하기 →
              </button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", gap: 8, padding: "16px 14px" }}>
              {[
                { icon: <Ic.Bell />, title: "혼잡 알림", sub: "임계치 초과 시 즉시 푸시 알림" },
                { icon: <Ic.Dash />, title: "구역별 체류 분석", sub: "진열대 앞 평균 체류 2.3배 ↑" },
                { icon: <Ic.Users />, title: "그룹 방문 비율", sub: "2인 이상 방문이 전체의 58%" },
              ].map((item, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: "#fff", borderRadius: 10, border: "1px solid var(--line)" }}>
                  <div style={{ width: 34, height: 34, borderRadius: 9, flexShrink: 0, background: "var(--accent-soft)", display: "grid", placeItems: "center", color: "var(--accent-ink)" }}>
                    {item.icon}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: "var(--ink)", letterSpacing: "-0.01em" }}>{item.title}</div>
                    <div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 2 }}>{item.sub}</div>
                  </div>
                  <div style={{ width: 44, height: 18, borderRadius: 6, flexShrink: 0, background: "var(--line-2)", filter: "blur(5px)" }} />
                </div>
              ))}
            </div>
          </div>
          ...
          {t.showPrivacyBadge && (
            <div className="priv" style={{ padding: "6px 4px 12px", fontSize: 12 }}>
              ...
              본 대시보드는 Vision AI 기반 익명 집계 데이터만 표시합니다. 얼굴 인식, 개인 식별은 수행하지 않으며 모든 처리는 백엔드에서 수치화 후 폐기됩니다.
            </div>
          )}
        </div>
      </div>
